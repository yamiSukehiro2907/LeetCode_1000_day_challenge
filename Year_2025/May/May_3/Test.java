...
public class Test {
    ...
    static class Solution {
        public int minDominoRotations(int[] tops, int[] bottoms) {
            ...
            for (int num : tops)
                topMap.put(num, topMap.getOrDefault(num, 0) + 1);
            for (int num : bottoms)
                bottomMap.put(num, bottomMap.getOrDefault(num, 0) + 1);
            ...
            for (int num : map.keySet()) {
                ...
                for (int i = 0; i < topLength; i++) {
                    ...
                    if (tops[i] == num) { // If the number is on top
                        if (!top)
                            temp++; // if the maxFreq of number is bottom then flip
                    } else if (bottoms[i] == num) { // If the number is on bottom
                        if (top)
                            temp++; // if the maxFreq of number is on top then flip
                    } else {
                        Notpossible = true;
                        break;
                    }
