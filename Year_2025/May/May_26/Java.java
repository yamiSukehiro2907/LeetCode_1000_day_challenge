...
class Solution {
    public int largestPathValue(String colors, int[][] edges) {
        ...
        int freq[][] = new int[n][26];
        for(int i = 0 ; i < n && ans != Integer.MAX_VALUE ; i++) ans = Math.max(ans , solve(i , colors , graph , freq , visited));
        return ans == Integer.MAX_VALUE ? -1 : ans;
    }
    private int solve(int index , String colors , List<List<Integer>> graph , int[][] freq , int[] visited){
        ...
        for(int adj : graph.get(index)){
            ...
            for(int letter = 0 ; letter < 26 ; letter++) freq[index][letter] = Math.max(freq[index][letter] , freq[adj][letter]);
        }
